...
resource "aws_s3_bucket" "primary-backend-bucket" {
  ...
  bucket = "aft-backend-${random_string.bucket_name.result}-primary-region"

  tags = {
    "Name" = "aft-backend-${random_string.bucket_name.result}-primary-region"
  }
}
...
resource "aws_s3_bucket" "secondary-backend-bucket" {
  ...
  bucket   = "aft-backend-${random_string.bucket_name.result}-secondary-region"
  tags = {
    "Name" = "aft-backend-${random_string.bucket_name.result}-secondary-region"
  }
}
...
resource "aws_dynamodb_table" "lock-table" {
  ...
  name             = "aft-backend-${random_string.bucket_name.result}"
...
resource "aws_kms_key" "encrypt-primary-region" {
  ...
  tags = {
    "Name" = "aft-backend-${random_string.bucket_name.result}-primary-region-kms-key"
  }
}
...
resource "aws_kms_alias" "encrypt-alias-primary-region" {
  ...
  name          = "alias/aft-backend-${random_string.bucket_name.result}-kms-key"
...
resource "aws_kms_key" "encrypt-secondary-region" {
  ...
  enable_key_rotation     = "true"
  tags = {
    "Name" = "aft-backend-${random_string.bucket_name.result}-secondary-region-kms-key"
  }
}
